...
from arbiter import Arbiter, Task, MockEventNode
...
class TestArbiter:
    ...
    @classmethod
    def setup_class(cls):
        cls.event_node = MockEventNode()
        cls.event_node.start()
        cls.minion = start_minion(cls.event_node)

    @classmethod
    def teardown_class(cls):
        stop_minion(cls.minion)
        cls.event_node.stop()

    @staticmethod
    def test_task_in_task():
        tasks_in_task_in_task = 5
        arbiter = Arbiter(event_node=TestArbiter.event_node)
